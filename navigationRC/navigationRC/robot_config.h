...
// (navigationRC copy — full RC link kept (both E-stop paths active).
// Joysticks are NOT used to drive in this sketch; see NAV_TRIGGER_CH
// below for the switch that starts the pre-programmed route.)
//
...
// ------------------------------------------------------------
// NAVIGATION — scripted step sequencer (see navigation.h / navigation.cpp)
//
// Runs a fixed array of {duration, direction, target heading} steps.
// TIME-based only (no encoder distance, no X/Y) — that's the deliberate
// scope for this stage, ahead of real coordinate-based navigation.
// ------------------------------------------------------------
#define NAV_DEFAULT_SPEED          0.35f   // normalized drive speed when a step's speed is 0
#define NAV_TURN_KP                1.0f    // Omega per rad of heading error (turn-to-target)
#define NAV_TURN_MAX_OMEGA         0.40f   // clamp on the turn/correction Omega, -1..1 scale
#define NAV_HEADING_TOLERANCE_DEG  5.0f    // "close enough" to the target heading to start driving
#define NAV_TURN_TIMEOUT_MS        4000UL  // give up waiting to reach heading and drive anyway

// ------------------------------------------------------------
// NAV TRIGGER — which RC channel starts the loaded route (navigationRC.ino).
//
// CH2 is the left stick's up/down axis, which THIS transmitter reports
// as having NO spring return (see the CHANNEL MAPPING note above) — it
// stays wherever you last left it, so it behaves like a toggle switch
// rather than something you have to hold. That's why it's picked as the
// default trigger instead of a momentary button.
//
// VERIFY THIS ON THE BENCH before relying on it in the field: watch the
// "[raw trigger ch:...]" value navigationRC.ino prints over Serial while
// you flip whichever button/switch you intend to use, and confirm it's
// actually CH2 that moves — FlySky transmitters vary in which physical
// switch maps to which channel. Change NAV_TRIGGER_CH here if not.
// ------------------------------------------------------------
#define NAV_TRIGGER_CH             2       // channel that starts/aborts the route
#define NAV_TRIGGER_THRESHOLD      1500    // raw value above this = "triggered"
