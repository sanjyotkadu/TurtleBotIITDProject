...
// STATUS INDICATOR — separate "is autonomous nav actually running" heartbeat
// (statusIndicator.h/.cpp), independent of OBSTACLE_LED_PIN above (that one
// means "something is in front of me right now"; this one means "a nav
// sequence is currently running at all"). Pin 23 is free in this map (see
// the Pin Allocation Summary below).
#define STATUS_INDICATOR_LED_PIN       23
#define STATUS_BLINK_NAV_MS           400   // slow steady blink while a nav sequence runs normally
#define STATUS_BLINK_OBSTACLE_MS      120   // fast blink while blocked/scanning/detouring around one
...
#define OBSTACLE_CHECK_CHUNK_MM    10.0f  // how far the forward watch drives between ultrasonic checks
                                           // while cruising straight - shrunk to match the single-digit
                                           // -cm thresholds above (a bigger gap could blow straight
                                           // through them before the next check). Safe to keep this
                                           // small because driveStraightWithObstacleCheck() checks
                                           // in-line on a CONTINUOUS drive (no stop/restart between
                                           // checks) - it does NOT drive this distance blind and then
                                           // halt like a separate navDriveStraight() call would.
...
#define OBSTACLE_PING_GAP_MS          12  // minimum gap between successive HC-SR04 trigger pulses
                                           // (used between pingClosestCm()'s samples and between
                                           // directionIsClear()'s pair of checks). The earlier 3-5ms
                                           // gap was too tight - back-to-back pings on the same sensor
                                           // can still be hearing the PREVIOUS ping's echo/reverb
                                           // rather than a fresh one, which reads as random noise
                                           // (missed real obstacles AND phantom "blocked" readings).
                                           // 12ms is short enough not to matter now that checks happen
                                           // in-line on a continuous drive instead of stopping for them.
...
//  Pin 23  → STATUS_INDICATOR_LED_PIN (nav-running heartbeat — external LED, spare pin)
