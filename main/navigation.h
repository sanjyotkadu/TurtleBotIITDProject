...
// One drive-command tick: kinematics -> PID -> DIR-corrected motor output,
// same pipeline main.ino's driveturBot() uses. obstacleAvoid.cpp's
// driveStraightWithObstacleCheck() calls this directly (instead of going
// through navDriveStraight()) so it can hold ONE continuous heading-lock
// and drive command across the whole leg, checking the sonar in-line
// without stopping/restarting the motors at every check - see that
// function's header comment for why that stop/restart pattern is exactly
// what caused the "zigzag" driving that motivated pulling this out.
void navDrive(float Vx, float Vy, float Omega);

// Ground distance covered (mm) since the A/B encoder counts were
// startA/startB, i.e. fabs(delta)-averaged and projected from wheel-rolled
// distance to chassis distance (see NAV_WHEEL_TRAVEL_RATIO in
// navigation.cpp). Shared so obstacleAvoid.cpp's continuous drive loop
// tracks progress the exact same way navDriveStraight() does, instead of
// a second copy of the conversion.
