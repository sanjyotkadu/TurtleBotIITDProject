...
void debugPrint(bool enabled, bool failsafe) {
  ...
  Serial.print(imu_pitchDeg(), 1);
  Serial.print(" roll:");
  Serial.print(imu_rollDeg(), 1);
  Serial.print(" healthy:");
  Serial.print(imu_healthy() ? "Y" : "N");
  Serial.print(" tipped:");
  Serial.print(imu_isTipped() ? "Y" : "N");
  Serial.print(" resets:");
  Serial.print(imu_resetCount());
  Serial.println("]");

  // Heading-hold debug: shows whether it's actually converging back to
  // the latched target, or stalling short of it (e.g. correction shrinks
  // below what the motors/PID deadband will actually move on).
  Serial.print("  [headingHold locked:");
  Serial.print(imu_headingHoldLocked() ? "Y" : "N");
  Serial.print(" current:");
  Serial.print(imu_headingDeg(), 1);
  Serial.print(" target:");
  Serial.print(imu_headingHoldTargetDeg(), 1);
  Serial.print(" error:");
  Serial.print(imu_headingHoldErrorDeg(), 1);
  Serial.print(" correction:");
  Serial.print(imu_headingHoldCorrection(), 3);
  Serial.println("]");
}
