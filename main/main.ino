 ...
 *   statusIndicator.*  — non-blocking LED heartbeat: blinks while a nav
 *                        sequence is running, switches to a faster blink
 *                        while obstacleAvoid.* has something in the way.
 *
 * Control flow:
...
void loop() {
  ...
  statusIndicatorUpdate(); // advance the nav-running LED blink; no-op while idle
  ...
  if (navTriggerHigh && navTriggerReady && enabled) {
    ...
    statusIndicatorSetState(STATUS_IDLE);
