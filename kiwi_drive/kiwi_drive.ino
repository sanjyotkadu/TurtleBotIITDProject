...
float dbgPowerC = 0, dbgPowerA = 0, dbgPowerB = 0; // normalized, pre-direction-correction
int dbgPwmC = 0, dbgPwmA = 0, dbgPwmB = 0;         // actual PWM (0-255) sent to each motor
...
bool sticksAreCentered() {
  ...
  float om = normalizedStick(IBUS_CH_OMEGA, centerOmega);
...
void driveKiwi(float Vx, float Vy, float Omega) {
  ...
  dbgPowerC = motorOnePower;
  dbgPowerA = motorTwoPower;
  dbgPowerB = motorThreePower;
  ...
  dbgPwmC = (int)(fabs(motorOnePower)   * MOTOR_PWM_MAX);
  dbgPwmA = (int)(fabs(motorTwoPower)   * MOTOR_PWM_MAX);
  dbgPwmB = (int)(fabs(motorThreePower) * MOTOR_PWM_MAX);
