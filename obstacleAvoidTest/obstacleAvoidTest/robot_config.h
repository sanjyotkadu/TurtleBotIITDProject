...
// SERVO — SG90 (test)
//
// Signal only needs a free digital pin (the Servo library drives it with
// a hardware interval timer, not analogWrite PWM). Pin 20 is unused by
// anything else in this map.
//
// POWER — do NOT run the signal wire's 5V/VCC pin off the Teensy's own
// 3.3V pin (SG90 needs ~4.8-6V and the 3.3V rail is too low and too
// current-limited). See the wiring note below for USB vs. external power.
//
// Wiring:
//   SG90 signal (orange) -> Teensy pin 20
//   SG90 VCC (red)        -> 5V (see power note)
//   SG90 GND (brown)      -> Teensy GND (common ground with whatever
//                             supplies VCC)
// ------------------------------------------------------------
#define SERVO_PIN        20
...
// Smooth-move slew rate (see servoMoveToSmooth() in obstacleAvoidTest.ino).
// writeMicroseconds() alone jumps straight to the target — the SG90 then
// slews there at its own max speed (~60 deg per ~0.1-0.15s, i.e. fast
// enough to look like a snap). Stepping 1 deg at a time with a delay
// between steps gives a visibly slower, controlled sweep instead.
// Time for a full 0-180 deg sweep ~= 180 * SERVO_SLEW_DELAY_MS.
// Larger delay = slower sweep; tune to taste.
#define SERVO_SLEW_STEP_DEG      1     // degrees per increment while slewing
#define SERVO_SLEW_DELAY_MS     20     // ms paused between increments

// ------------------------------------------------------------
// ULTRASONIC — HC-SR04 (test)
//
// TRIG is an output (Teensy 3.3V HIGH is well above the HC-SR04's input
// threshold, so it can drive TRIG directly). ECHO is an INPUT driven by
// the sensor at 5V — Teensy 4.x pins are NOT 5V tolerant, so ECHO MUST
// be stepped down before reaching the Teensy, or the pin can be
// damaged.
//
// Using a TXS0108E level-shifter module (marked "YF08E" on the chip —
// TI 8-channel bidirectional 3.3V<->5V logic translator) instead of a
// resistor divider — cleaner and auto-sensing direction. Only one
// channel is needed (ECHO); TRIG stays direct since 3.3V already reads
// as logic-high to the sensor.
//
//   TXS0108E VCCA -> Teensy 3.3V        TXS0108E VCCB -> 5V
//   TXS0108E OE   -> Teensy 3.3V (enable)
//   TXS0108E GND  -> common GND (both LV and HV GND pins)
//   TXS0108E A1   -> Teensy ECHO_PIN    TXS0108E B1   -> HC-SR04 ECHO
//
// Wiring:
//   HC-SR04 VCC  -> 5V
//   HC-SR04 GND  -> Teensy GND
//   HC-SR04 TRIG -> Teensy pin 21 (direct)
//   HC-SR04 ECHO -> TXS0108E B1 (5V side); TXS0108E A1 -> Teensy pin 22
// ------------------------------------------------------------
#define TRIG_PIN                21
#define ECHO_PIN                22
#define SONAR_TIMEOUT_US      30000   // pulseIn() timeout (~5m round trip max)
#define SONAR_MAX_RANGE_CM      400   // HC-SR04 datasheet max range

// Measured on the bench: this unit reads cleanly above 5cm but is
// unreliable below 4cm. This is normal HC-SR04 behavior, not a wiring
// or code issue — right after the trigger burst the transducer is
// still mechanically "ringing," and the receiver can't tell a genuine
// close-range echo from that residual ringing until it settles. The
// datasheet's 2cm minimum is optimistic; most real modules (especially
// clones) have an actual floor in the 3-5cm range. Readings below this
// are reported as "too close" rather than trusted.
#define SONAR_MIN_RANGE_CM        4   // empirically measured reliable floor
...
// ------------------------------------------------------------
// OBSTACLE-AVOIDANCE SCAN TEST (see obstacleAvoidTest.ino)
//
// Watch behaviour: servo pings whatever direction it is CURRENTLY
// pointed at (90 deg = straight ahead at boot, or wherever the last scan
// left it parked - it stays there for good, no periodic recentering). A
// scan triggers when THAT reading drops below OBSTACLE_TRIGGER_CM (or
// reads "too close", i.e. below SONAR_MIN_RANGE_CM above) - whether
// that's the very first obstacle in front, or a new one appearing at
// wherever it was previously parked.
//
// SCAN LOGIC — a single continuous glide, not a full stop-and-measure
// sweep (an earlier version stopped dead at every step to average pings,
// which is what made the motion look jerky/"zig-zag"). From center (90
// deg), glide toward the LEFT limit checking for an opening every
// SCAN_STEP_DEG of travel using a fast, short-timeout ping:
//   - opening found  -> stop right there, that's the way to go.
//   - reaches the LEFT limit with no opening -> keep gliding (same
//     continuous motion) across center to the RIGHT limit, same check.
//     - opening found -> stop there.
//     - reaches the RIGHT limit with no opening -> glide back to center
//       (90) and set the "blocked on all sides" flag (see
//       obstacleIsBlockedAllSides() in obstacleAvoidTest.ino) - this is
//       the flag the main navigation project will read to know it has to
//       try something other than driving forward.
//
// SWEEP RANGE — a single SG90 only travels its mechanical 0-180 deg, it
// cannot look behind the robot, so a true -180..180 (360 deg) scan isn't
// physically possible with one hobby servo. "LEFT limit"/"RIGHT limit"
// above are the servo's own 180 deg / 0 deg ends, reported as a relative
// angle in -90..+90 deg (left/right of straight ahead). Mount the servo
// so its 0-180 deg travel covers the robot's forward hemisphere.
// ------------------------------------------------------------
#define OBSTACLE_TRIGGER_CM       30.0f  // forward distance (cm) that starts a scan
#define SCAN_CLEAR_CM             40.0f  // distance (cm) needed to call a direction "clear" -
                                          // kept a bit above OBSTACLE_TRIGGER_CM (simple
                                          // hysteresis) so the result doesn't flicker right at
                                          // the trigger boundary
#define SCAN_STEP_DEG             10     // re-check for an opening every this many degrees of travel
#define SCAN_CONFIRM_STEPS         2     // consecutive clear check-points needed before committing to
                                          // "opening found" and stopping there. A narrow gap in the
                                          // ultrasonic beam, or an obstacle at an angle that reflects
                                          // the ping away instead of back, can make ONE check-point
                                          // read clear even though the arc right after it is still
                                          // blocked - requiring a couple of check-points in a row
                                          // (SCAN_STEP_DEG apart each) filters that out before the
                                          // scan commits to a direction and skips checking the other side.
#define SCAN_CHECK_TIMEOUT_US     6000   // short pulseIn() timeout for in-glide checks (~100cm
                                          // range - plenty above SCAN_CLEAR_CM). Far shorter than
                                          // SONAR_TIMEOUT_US below so a check barely pauses the
                                          // glide (a long timeout is what made the old design's
                                          // per-angle stop look like a stutter)
#define SCAN_REPORT_COOLDOWN_MS   2000   // pause after a scan resolves before the watch loop
                                          // starts checking the angle it parked at (lets a
                                          // transient noisy reading settle before it can retrigger)

// Maps servo angle (0-180, 90=straight ahead) to a robot-relative heading
// in degrees (-90..+90), CCW-positive to match IMU_YAW_SIGN's convention
// above (imu_headingDeg() increases CCW). Also decides which physical end
// of the servo's travel is "LEFT" for the scan logic above: +1 -> LEFT is
// toward 180 deg, RIGHT is toward 0 deg; -1 -> reversed. VERIFY on the
// bench: swinging the servo toward the robot's LEFT should print a
// POSITIVE relative angle in the scan report. Flip this to -1 if it
// comes out backwards.
#define SCAN_DIR_SIGN             (+1)

// Onboard LED (Teensy 4.1 pin 13 — no extra wiring needed): a quick blink
// burst the instant the idle watch trips, then solid ON for as long as
// obstacleIsBlockedAllSides() is true (blocked on every direction tried),
// OFF once an opening is found.
#define OBSTACLE_LED_PIN         13
#define OBSTACLE_ALERT_BLINKS      3    // quick blinks when idle watch first trips
...
//  Pin 13  → OBSTACLE_LED_PIN (Teensy onboard LED — obstacle alert blink)
//  Pin 20  → SERVO_PIN (SG90 signal)
//  Pin 21  → TRIG_PIN  (HC-SR04 trigger, direct)
//  Pin 22  → ECHO_PIN  (HC-SR04 echo, THROUGH TXS0108E LEVEL SHIFTER — 5V signal)
